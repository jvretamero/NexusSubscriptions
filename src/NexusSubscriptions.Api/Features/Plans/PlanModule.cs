...
public static class PlanModule
{
    public static IServiceCollection AddPlanModule(this IServiceCollection services)
    {
        ...
        services.AddScoped<ICommandHandler<DeletePlanRequest, DeletePlanResponse>, DeletePlanHandler>();
    ...
    private static async Task<IResult> DeletePlan(
        [FromRoute] int id,
        [FromServices] ICommandHandler<DeletePlanRequest, DeletePlanResponse> handler,
        CancellationToken ct)
    {
        var request = new DeletePlanRequest(id);
        var response = await handler.HandleAsync(request, ct);
    ...
    private static async Task<IResult> UpdatePlan(
        [FromRoute] int id,
        [FromBody] UpdatePlanDTO body,
        [FromServices] ICommandHandler<UpdatePlanRequest, UpdatePlanResponse> handler,
        CancellationToken ct)
    {
        var request = new UpdatePlanRequest(id, body.Description, body.Price);
        var response = await handler.HandleAsync(request, ct);
